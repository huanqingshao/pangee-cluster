...
func PopulateKuboardSprayVars(inventory map[string]interface{}, ownerType, ownerName string) {
	...
	MapSet(inventory, "all.vars.kuboardspray_ssh_multiplexing", "-o ControlMaster=no -o ControlPersist=60m -o ControlPath={{ kuboardspray_cluster_dir }}/{{ansible_user}}@{{ansible_host}}-{{ansible_port}}")
}
