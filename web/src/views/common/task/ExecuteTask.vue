...
<template>
  <div v-if="dialogVisible" style="display: inline-block; width: 0px; height: 0px;">
    <el-dialog v-model="dialogVisible" :title="t('processingTitle')" width="50%" :close-on-click-modal="false"
      :append-to-body="true" :show-close="false">
      <el-alert :type="type === 'primary' ? 'info' : type" :closable="false" :title="t('processingTitle')" show-icon
        effect="dark" style="margin-top: 20px;">
    ...
    <el-popover v-if="!(finished && hideOnSuccess) && !history.processing" v-model:visible="showConfirm"
      ...
      <el-form @submit.prevent.stop label-position="left" label-width="120px">
        <template v-if="finished">
          <el-alert :type="type === 'primary' ? 'info' : type" effect="dark" style="margin-bottom: 10px;"
            :closable="false">
