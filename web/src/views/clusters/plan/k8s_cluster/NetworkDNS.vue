...
<template>
  <ConfigSection
    ...
    disabled
    anti-freeze
    helpLink="https://kuboard.cn/guide/options/dns.html"
  >
    <template #more> 集群 DNS 设置 </template>
    <FieldArray
      :holder="vars"
      :prop="prop"
      newItemTemplate=""
      fieldName="upstream_dns_servers"
      :itemRules="upstreamDnsServerItemRules"
      anti-freeze
    >
      <template v-slot:editItem="scope">
        <el-input
          v-model.trim="vars.upstream_dns_servers[scope.index]"
          :placeholder="t('upstream_dns_servers_placeholder')"
        ></el-input>
      </template>
      <template #help>
    ...
    <ConfigSection v-model:enabled="nodeLocalDnsEnabled" :label="t('nodelocaldnsLabel')" labelWidth="120px">
      <template #more> 在每个节点上设置一个 DNS 服务，对 DNS 解析结果进行缓存，减少对 CoreDNS 的访问次数。 </template>
      <FieldString
        :holder="vars"
        fieldName="nodelocaldns_ip"
        :prop="prop"
        :rules="nodelocaldns_ip_rules"
        :helpString="t('nodelocaldnsHelpStr')"
      ></FieldString>
    </ConfigSection>
  </ConfigSection>
</template>
...
export default {
  ...
  computed: {
    ...
    vars: {
      get() {
        return this.cluster.inventory.all.children.target.children.k8s_cluster.vars;
      },
      set() {}
    },
    enabled: {
    ...
    coredns_external_zones: {
      get() {
        return this.vars.coredns_external_zones;
      },
      set() {}
    }
  },
  ...
  methods: {
    ...
    applyDnsConfig() {
      ...
      return new Promise((resolve, reject) => {
        ...
        this.kuboardSprayApi
          .post(`/clusters/${this.cluster.name}/install_addon`, req)
          .then(resp => {
