...
window.pangeecluster_github_star_action = function () {
  window.pangeecluster_github_star_ref.close()
  localStorage.setItem('pangeecluster_github_star_visited', 'true')
}

function showTip() {
  if (localStorage.getItem('pangeecluster_github_star_visited') === 'true') {
    return
  }
  window.pangeecluster_github_star_ref = ElNotification({
    title: '加关注，不迷路',
    dangerouslyUseHTMLString: true,
    // message: '<a onClick="pangeecluster_open_github_star()">This is <i>HTML</i> string</a>',
    message: `<div class="app_text_mono">加 Github star / watch，可以随时获取本项目的源代码、文档</div>
              <a href="https://github.com/opencmit/pangee-cluster" target=_blank class="app_notify" style="width: 100%;" onclick="window.pangeecluster_github_star_action()">
    ...
    type: 'warning',
    duration: 0,
    'custom-class': 'pangee-cluster-upgrade-notification',
  })
}
