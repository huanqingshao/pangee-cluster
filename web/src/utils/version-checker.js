...
async function init () {
    axios.get(`https://addons.kuboard.cn/pangee-cluster-latest-version.json?r=${Math.random()}`, {
      timeout: 2000,
      withCredentials: false,
    }).then(resp => {
      let installed = window.PangeeCluster.version.version
      ...
      if (installed !== newVersion) {
        ...
        setTimeout(() => {
            window.$kuboardVersionCheckNotification = window.VueAppComponent.$notify({
              ...
              message: `<a href="https://pangee-cluster.cn/support/change-log/v1.html" target=_blank class="app_notify" style="width: 100%;" onclick="window.$kuboardVersionCheckNotification.close()">
